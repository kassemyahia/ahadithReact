import { useId } from 'react'

export default function Select({ id, label, error, hint, children, className = '', required, ...props }) {
  const generatedId = useId()
  const selectId = id || generatedId
  const hintId = hint ? `${selectId}-hint` : undefined
  const errorId = error ? `${selectId}-error` : undefined

  return (
    <label className="block" htmlFor={selectId}>
      {label && (
          <span className="mb-1.5 block text-sm font-bold text-[var(--color-text)]">
          {label}
          {required && <span aria-hidden="true"> *</span>}
        </span>
      )}
      <select
        id={selectId}
        required={required}
        className={`min-h-12 w-full rounded-[16px] border border-[var(--color-border-gold)] bg-white px-4 py-2 text-[var(--color-text)] outline-none transition focus:border-[var(--color-primary-strong)] focus:ring-4 focus:ring-[var(--color-primary-soft)] ${className}`}
        aria-invalid={Boolean(error)}
        aria-describedby={[hintId, errorId].filter(Boolean).join(' ') || undefined}
        {...props}
      >
        {children}
      </select>
      {hint && <span id={hintId} className="mt-1 block text-sm text-[var(--color-text-muted)]">{hint}</span>}
      {error && <span id={errorId} className="mt-1 block text-sm text-[var(--color-danger)]">{error}</span>}
    </label>
  )
}
